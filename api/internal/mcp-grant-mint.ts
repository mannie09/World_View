 ...
 * `worldview.app/mcp-grant` to the api-subdomain consent flow at
 * `api.worldview.app/oauth/authorize-pro` (U5).
 *
 * Flow:
 ...
 *
 * The redirect URL host is FIXED to `https://api.worldview.app` —
...
const AUTHORIZE_PRO_URL = 'https://api.worldview.app/oauth/authorize-pro';
...
export async function mintGrantHandler(req: Request, deps: MintDeps): Promise<Response> {
  ...
  if (
    ...
    return jsonError('INSUFFICIENT_TIER', 'A WorldView Pro subscription is required.', 403);
  }
