...
function makeReportRequest({ body, contentType = 'application/reports+json' } = {}) {
  return new Request('https://worldview.app/api/security/report', {
    method: 'POST',
...
test('security report endpoint accepts Reporting API batches and redacts URLs in logs', async (t) => {
    ...
    body: JSON.stringify([
      {
        type: 'coep',
        age: 10,
        url: 'https://tech.worldview.app/panel?token=secret',
        body: {
  ...
  assert.match(logs[0], /"urlOrigin":"https:\/\/tech\.worldview\.app"/);
...
test('security report endpoint rejects unsupported methods', async () => {
  const response = await handler(new Request('https://worldview.app/api/security/report'));
