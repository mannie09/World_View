...
function renderErrorPage(heading: string, body: string): string {
  return (
    ...
    + `<h1>${heading}</h1>`
    + `<p>${body}</p>`
    + '<p><a href="https://worldview.app">Return to WorldView</a></p>'
    + '</div></body></html>'
  );
}
...
const FORBIDDEN_PAGE = renderErrorPage(
  ...
  "The brief link you followed is incomplete or has been tampered with. Open the most recent notification from WorldView to read today's brief.",
);
  ...
  try {
    const res = await fetch(`${convexSiteUrl}/relay/followed-countries`, {
      ...
      headers: {
        ...
        'User-Agent': 'worldview-brief-render/1.0',
      },
