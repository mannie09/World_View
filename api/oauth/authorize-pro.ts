 ...
 *      row. NO `wm_` key, NO `WORLDVIEW_VALID_KEYS` write — Pro identity
...
function htmlError(title: string, detail: string, status: number = 400): Response {
  return new Response(
    `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Error &#x2014; WorldView MCP</title>
...
export async function authorizeProHandler(req: Request, deps: AuthorizeProDeps): Promise<Response> {
  ...
  if (
    ...
    return htmlError(
      ...
      'A WorldView Pro subscription is required for this connection. Please subscribe and try again.',
      403,
    );
  }
