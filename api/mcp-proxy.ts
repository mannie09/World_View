...
function buildInitPayload() {
  return {
    ...
    params: {
      ...
      capabilities: {},
      clientInfo: { name: 'worldview', version: '1.0' },
    },
  };
}
...
function buildHeaders(customHeaders) {
  const h = {
    ...
    'User-Agent': 'WorldView-MCP-Proxy/1.0',
  };
...
export default async function handler(req) {
  ...
  // (Authorization, API keys) to any public MCP server under WorldView's
  ...
  // accepts: enterprise key (WORLDVIEW_VALID_KEYS), wm_ user API key
