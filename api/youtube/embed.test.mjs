...
function makeRequest(query = '') {
  return new Request(`https://worldview.app/api/youtube/embed${query}`);
}
...
test('returns embeddable html for valid video id', async () => {
  ...
  assert.equal(html.includes('origin:"https://worldview.app"'), true);
...
test('uses dedicated parentOrigin for iframe postMessage target', async () => {
  const response = await handler(makeRequest('?videoId=iEpJwprxDdk&origin=https://worldview.app&parentOrigin=https://tauri.localhost'));
  const html = await response.text();
  assert.match(html, /playerVars:\{[^}]*origin:"https:\/\/worldview\.app"/);
...
test('does not accept wildcard parentOrigin query parameter', async () => {
  const response = await handler(makeRequest('?videoId=iEpJwprxDdk&origin=https://worldview.app&parentOrigin=*'));
  const html = await response.text();
  assert.equal(html.includes('parentOrigin="*"'), false);
  assert.match(html, /parentOrigin="https:\/\/worldview\.app"/);
});
