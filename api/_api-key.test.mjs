...
function makeReq({ origin, referer, secFetchSite, key, cookie } = {}) {
  ...
  if (key) headers.set('x-worldview-key', key);
  if (cookie) headers.set('cookie', cookie);
  return new Request('https://api.worldview.app/api/test', { headers });
}

// ── #3541 regression: header-only signals must NEVER pass ──────────────────

test('#3541: forged Referer alone is rejected', async () => {
  const r = await validateApiKey(makeReq({ referer: 'https://worldview.app/' }));
...
test('#3541: forged Origin: https://worldview.app alone is rejected (no key, no session)', async () => {
  const r = await validateApiKey(makeReq({ origin: 'https://worldview.app' }));
  assert.equal(r.valid, false);
});
...
// ── Enterprise key (WORLDVIEW_VALID_KEYS) ────────────────────────────────
...
test('REGRESSION: wm_-prefixed key in WORLDVIEW_VALID_KEYS is honored as enterprise', async () => {
  // Pre-#3541 the static enterprise allowlist accepted any key shape, so
  // some operator-issued keys (e.g. Railway WORLDVIEW_RELAY_KEY) carry the
  ...
  // present in WORLDVIEW_VALID_KEYS.
  const previous = process.env.WORLDVIEW_VALID_KEYS;
  process.env.WORLDVIEW_VALID_KEYS = `${ENTERPRISE_KEY},wm_legacy_operator_key_in_static_list`;
  try {
    ...
    process.env.WORLDVIEW_VALID_KEYS = previous;
  }
});
