...
const DESKTOP_AUTH_TIMESTAMP_HEADER = 'X-WorldView-Desktop-Timestamp';
const DESKTOP_AUTH_SIGNATURE_HEADER = 'X-WorldView-Desktop-Signature';
...
const ALLOW_PRIVATE_NETWORK_FETCH = Symbol('worldview.allowPrivateNetworkFetch');
...
const ALLOWED_ENV_KEYS = new Set([
  ...
  'OLLAMA_API_URL', 'OLLAMA_MODEL', 'WORLDVIEW_API_KEY', 'WTO_API_KEY',
...
async function proxyToCloud(requestUrl, req, remoteBase) {
  ...
  headers.set('Origin', 'https://worldview.app');
  ...
  // supplied X-WorldView-Key (e.g. a wm_ user key from runtime config) wins
  // — don't clobber it.
  if (!headers.has('X-WorldView-Key')) {
    const wmKey = process.env.WORLDVIEW_API_KEY;
    if (wmKey) headers.set('X-WorldView-Key', wmKey);
  }
  return fetch(target, {
    method: req.method,
    headers,
...
function resolveConfig(options = {}) {
  ...
  const remoteBase = String(options.remoteBase ?? process.env.LOCAL_API_REMOTE_BASE ?? 'https://api.worldview.app').replace(/\/$/, '');
  ...
  if (mode === 'docker' && requestedFallback) {
    logger.warn('[local-api] Cloud fallback disabled in Docker mode (self-hosted instances must not proxy to api.worldview.app)');
  }
...
const SIDECAR_ALLOWED_ORIGINS = [
  ...
  // Only allow exact domain or single-level subdomains (e.g. preview-xyz.worldview.app).
  // The previous (.*\.)? pattern was overly broad. Anchored to prevent spoofing
  // via domains like worldviewEVIL.vercel.app.
  /^https:\/\/([a-z0-9-]+\.)?worldview\.app$/,
];
...
function makeCorsHeaders(req) {
  return {
    ...
    'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-WorldView-Desktop-Timestamp, X-WorldView-Desktop-Signature',
