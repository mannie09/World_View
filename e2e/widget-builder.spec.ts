...
test.describe('AI widget builder', () => {
  test.beforeEach(async ({ page }) => {
    await page.addInitScript((key) => {
      if (!sessionStorage.getItem('__widget_e2e_init__')) {
        ...
        localStorage.setItem('worldview-variant', 'happy');
  ...
  test('creates a widget through the live modal flow and persists it after reload', async ({ page }) => {
    ...
    await installWidgetAgentMocks(
      ...
      [
        {
          delayMs: 250,
          endpoint: '/rpc/worldview.markets.v1.MarketsService/GetCommodities',
          title: 'Oil vs Gold',
          html: createHtml,
        },
      ],
  ...
  test('supports modify, keeps session history, exposes touch-sized controls, and cleans storage on delete', async ({ page }) => {
    ...
    await installWidgetAgentMocks(page, [
      ...
      {
        endpoint: '/rpc/worldview.aviation.v1.AviationService/GetAirportDelays',
    ...
    await page.evaluate((widgetId: string) => {
      localStorage.setItem('worldview-panel-spans', JSON.stringify({ [widgetId]: 2 }));
      localStorage.setItem('worldview-panel-col-spans', JSON.stringify({ [widgetId]: 3 }));
    }, storedWidgetMeta!.id);
    ...
    const cleanedStorage = await page.evaluate(() => {
      return {
        ...
        rowSpans: localStorage.getItem('worldview-panel-spans'),
        colSpans: localStorage.getItem('worldview-panel-col-spans'),
      };
    });
