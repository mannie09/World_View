...
test.describe('desktop runtime routing guardrails', () => {
  ...
  test('runtime fetch patch falls back to cloud for local failures', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      await runtimeConfig.setSecretValue('WORLDVIEW_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, 'wm_test_key_1234567890abcdef');
      ...
      try {
        ...
        await runtimeConfig.setSecretValue('WORLDVIEW_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, '');
      }
    });
  ...
  test('update badge picks architecture-correct desktop download url', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const previousTauri = globalWindow.__TAURI__;
      const releaseUrl = 'https://github.com/mannie09/World_View/releases/latest';
    ...
    expect(result.windowsX64).toBe('https://worldview.app/api/download?platform=windows-exe&variant=full');
    expect(result.linuxFallback).toBe('https://github.com/mannie09/World_View/releases/latest');
  });
  ...
  test('cloud fallback blocked without WorldView API key', async ({ page }) => {
    await page.goto('/tests/runtime-harness.html');
  ...
  test('cloud fallback allowed with valid WorldView API key', async ({ page }) => {
    await page.goto('/tests/runtime-harness.html');
    ...
    const result = await page.evaluate(async () => {
      ...
      await runtimeConfig.setSecretValue('WORLDVIEW_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, testKey);
