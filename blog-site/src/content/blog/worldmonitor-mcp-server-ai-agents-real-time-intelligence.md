---
title: "Give Your AI Agent Eyes on the World: The WorldView MCP Server"
...
metaTitle: "Real-Time Intelligence MCP Server | WorldView"
...
The Model Context Protocol (MCP) fixes the plumbing problem: it gives AI assistants a standard way to call live tools. WorldView fixes the data problem: it exposes the entire intelligence platform, the same one behind the [free real-time dashboard](/blog/posts/what-is-worldview-real-time-global-intelligence/), as an MCP server with **39 live tools**.
...
{
  "mcpServers": {
    "worldview": { "url": "https://worldview.app/mcp" }
  }
}
```

**Claude web (claude.ai):** Settings → Connectors → Add custom connector → name it WorldView, paste `https://worldview.app/mcp`.
...
```bash
npx @modelcontextprotocol/inspector https://worldview.app/mcp
```

On first use, the client walks you through "Sign in with WorldView Pro." MCP access requires a [Pro account](https://www.worldview.app/pro); API Starter and Enterprise keys (`X-WorldView-Key`) also work for server-to-server agents. OAuth clients do not need copied keys because the protocol handles token exchange and refresh.
...
Every WorldView tool accepts an optional `jmespath` argument. The server applies the expression server-side and returns only the projection:
...
WorldView treats autonomous agents as first-class clients. From the root URL, an agent can discover everything by itself:
...
So an agent that has never heard of WorldView can start from `https://worldview.app/`, read the Link headers, and wire itself up without a human in the loop. If you prefer raw REST over MCP, the same data is available through the [developer API](/blog/posts/build-on-worldview-developer-api-open-source/), and the two share authentication.
...
**Does the WorldView MCP server work with ChatGPT or other non-Claude clients?**
...
**Add `https://worldview.app/mcp` to your MCP client and ask your agent what changed in the world today. Setup docs at [worldview.app/docs/mcp-quickstart](https://www.worldview.app/docs/mcp-quickstart).**
