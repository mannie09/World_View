...
const CATALOG_API = 'https://api.worldview.app/api/product-catalog';
...
function isInProductHref(href: string): boolean {
  ...
  try {
    const base = typeof window !== 'undefined' ? window.location.href : 'https://worldview.app/';
    const url = new URL(href, looksRelative ? base : undefined);
    if (url.protocol !== 'http:' && url.protocol !== 'https:') return false;
    return url.hostname === 'worldview.app' ||
           url.hostname.endsWith('.worldview.app') ||
...
function getCtaProps(tier: Tier, billing: 'monthly' | 'annual'): CtaProps {
  if (tier.cta && tier.href && tier.price === 0) {
    // Free tier on an in-product href (e.g. worldview.app dashboard)
