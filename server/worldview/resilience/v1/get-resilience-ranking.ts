...
} from '../../../../src/generated/server/worldview/resilience/v1/service_server';
...
async function isSeedRefreshAuthorized(ctx: ServerContext): Promise<boolean> {
  ...
  const candidate = ctx.request.headers.get('X-WorldView-Key') ?? '';
...
export const getResilienceRanking: ResilienceServiceHandler['getResilienceRanking'] = async (
  ...
  // (Pro bearer, WORLDVIEW_VALID_KEYS, WORLDVIEW_API_KEY) must keep the
