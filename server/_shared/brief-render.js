// Deterministic renderer for the WorldView Brief magazine.
//
...
//   - Brainstorm: docs/brainstorms/2026-04-17-worldview-brief-magazine-requirements.md
//   - Plan: docs/plans/2026-04-17-003-feat-worldview-brief-magazine-plan.md
...
function logoRef({ size, color }) {
  ...
  return (
    ...
    `aria-label="WorldView"${styleAttr}>` +
...
function digestRunningHead(dateShort, label) {
  return (
    ...
    logoRef({ size: 22 }) +
    ` · WorldView Brief · ${escapeHtml(dateShort)} ·` +
    '</span>' +
...
function buildTrackedSourceUrl(raw, issueDate, rank) {
  try {
    const u = new URL(raw);
    if (!u.searchParams.has('utm_source')) u.searchParams.set('utm_source', 'worldview');
...
function renderStoryPage({ story, rank, palette, pageIndex, totalPages, issueDate, followedSet }) {
  ...
  return (
    ...
    logoRef({ size: 28 }) +
    '<span class="mono">WorldView Brief</span>' +
    '</div>' +
    `<div class="page-number mono">${pad2(pageIndex)} / ${pad2(totalPages)}</div>` +
    '</section>'
  );
...
function renderBackCover({ tz, pageIndex, totalPages, publicMode, refCode }) {
  const ctaHref = publicMode
    ? `https://worldview.app/pro${refCode ? `?ref=${encodeURIComponent(refCode)}` : ''}`
    : 'https://worldview.app';
  ...
  const metaLeft = publicMode
    ...
    : '<span class="mono">worldview.app</span>';
...
const STYLE_BLOCK = `<style>
  :root {
    /* WorldView brand palette — aligned with /pro landing + dashboard.
...
(function() {
  ...
  btn.addEventListener('click', async function() {
    ...
    try {
      var shareTitle = 'WorldView Brief';
      var shareText = 'My WorldView Brief for today:';
      if (navigator.share) {
        try {
...
// index.html. Hosted magazine pages are served from worldview.app
...
const UMAMI_LOADER = '<script async src="https://abacus.worldview.app/script.js" data-website-id="e8800335-c853-46a8-8497-c993ed2f58bc" data-domains="worldview.app,tech.worldview.app,finance.worldview.app,commodity.worldview.app,happy.worldview.app"></script>';
...
function redactForPublic(data) {
  ...
  return {
    ...
    stories: data.stories.map((s) => ({
      ...s,
      whyMatters: 'Subscribe to WorldView Brief to see the full editorial on this story.',
    })),
  };
}
...
export function renderBriefMagazine(envelope, options = {}) {
  ...
  const title = `WorldView Brief · ${escapeHtml(dateLong)}`;
  ...
  const publicStripHref = `https://worldview.app/pro${refCode ? `?ref=${encodeURIComponent(refCode)}` : ''}`;
  const publicStripHtml = publicMode
    ? '<div class="wm-public-strip">'
      + '<span>WorldView Brief \u00b7 shared issue</span>'
