...
//
// Usage:
//   WORLDVIEW_API_KEY=wm_xxx API_BASE=https://api.worldview.app \
//     node scripts/audit-resilience-cohorts.mjs
//   WORLDVIEW_API_KEY=wm_xxx API_BASE=... \
...
// (see src/shared/premium-paths.ts). A valid WORLDVIEW_API_KEY is
...
if (!FIXTURE_PATH) {
  if (!API_BASE) {
    console.error('[audit-resilience-cohorts] API_BASE env var required (e.g. https://api.worldview.app), or FIXTURE=path.json for offline mode');
    process.exit(2);
  }
  if (!process.env.WORLDVIEW_API_KEY) {
    console.error('[audit-resilience-cohorts] WORLDVIEW_API_KEY env var required; resilience RPC paths are in PREMIUM_RPC_PATHS.');
    process.exit(2);
  }
}
...
function apiHeaders() {
  ...
  if (process.env.WORLDVIEW_API_KEY) {
    h['X-WorldView-Key'] = process.env.WORLDVIEW_API_KEY;
  }
  return h;
}
