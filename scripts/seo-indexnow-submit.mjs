#!/usr/bin/env node
/**
 * Submit all worldview.app URLs to IndexNow after deploy.
...
function getBlogPostUrls() {
  return readdirSync(BLOG_DIR)
    ...
    .map((file) => `https://www.worldview.app/blog/posts/${basename(file, '.md')}/`)
    .sort();
}

const WWW_URLS = [
  'https://www.worldview.app/',
  'https://www.worldview.app/pro',
  'https://www.worldview.app/blog/',
  ...getBlogPostUrls(),
];

const BATCHES = [
  {
    host: 'www.worldview.app',
    urls: WWW_URLS,
  },
  { host: 'tech.worldview.app', urls: ['https://tech.worldview.app/'] },
  { host: 'finance.worldview.app', urls: ['https://finance.worldview.app/'] },
  { host: 'happy.worldview.app', urls: ['https://happy.worldview.app/'] },
];
...
async function submit(endpoint, host, urlList) {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      'User-Agent': 'WorldView-IndexNow/1.0 (+https://www.worldview.app)',
    },
