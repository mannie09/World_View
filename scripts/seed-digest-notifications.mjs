...
// like `RESEND_FROM_BRIEF=brief@worldview.app` can't re-introduce the bug
...
const WORLDVIEW_PUBLIC_BASE_URL =
  process.env.WORLDVIEW_PUBLIC_BASE_URL ?? 'https://worldview.app';
...
const BRIEF_WHY_MATTERS_ENDPOINT_URL =
  ...
  `${WORLDVIEW_PUBLIC_BASE_URL}/api/internal/brief-why-matters`;

/**
 * Lowercase + collapse whitespace to mirror extractor-side gate in
 * server/worldview/news/v1/list-feed-digest.ts
  ...
  try {
    const resp = await fetch(BRIEF_WHY_MATTERS_ENDPOINT_URL, {
      ...
      headers: {
        ...
        'User-Agent': 'worldview-digest-notifications/1.0',
...
async function upstashRest(...args) {
  const res = await fetch(`${UPSTASH_URL}/${args.map(encodeURIComponent).join('/')}`, {
    ...
    headers: {
      ...
      'User-Agent': 'worldview-digest/1.0',
    },
...
function formatDigest(stories, nowMs) {
  ...
  const lines = [`WorldView Daily Digest — ${dateStr}`, ''];
  ...
  <div style="max-width: 680px; margin: 0 auto;">
    ...
    <div style="background: #0d0d0d; padding: 32px 36px 0;">
            ...
            <table cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td style="width: 36px; height: 36px; vertical-align: middle;">
                  <img src="https://www.worldview.app/favico/android-chrome-192x192.png" width="36" height="36" alt="WorldView" style="border-radius: 50%; display: block;" />
                </td>
      ...
      <div style="text-align: center; padding: 12px 0 36px;">
        <a href="https://worldview.app/dashboard" style="display: inline-block; background: #4ade80; color: #0a0a0a; padding: 12px 32px; text-decoration: none; font-weight: 700; font-size: 12px; text-transform: uppercase; letter-spacing: 1.5px; border-radius: 3px;">Open Dashboard</a>
      </div>
    </div>
    <div style="background: #0a0a0a; border-top: 1px solid #1a1a1a; padding: 20px 36px; text-align: center;">
      <div style="margin-bottom: 12px;">
        <a href="https://x.com/worldviewapp" style="color: #555; text-decoration: none; font-size: 11px; margin: 0 10px;">X / Twitter</a>
        <a href="https://github.com/mannie09/World_View" style="color: #555; text-decoration: none; font-size: 11px; margin: 0 10px;">GitHub</a>
...
async function sendTelegramBriefCarousel(userId, chatId, caption, magazineUrl) {
  ...
  try {
    const res = await fetch(
      ...
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'User-Agent': 'worldview-digest/1.0' },
...
async function getUserTier(userId) {
  ...
  try {
    const res = await fetch(`${CONVEX_SITE_URL}/relay/entitlement`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${RELAY_SECRET}`, 'User-Agent': 'worldview-digest/1.0' },
...
function buildChannelBodies(storyListPlain, aiSummary, magazineUrl) {
  ...
  const briefFooterPlain = magazineUrl
    ? `\n\n${DIVIDER}\n\n📖 Open your WorldView Brief magazine:\n${magazineUrl}`
    : '';
  const briefFooterTelegram = magazineUrl
    ? `\n\n${DIVIDER}\n\n📖 <a href="${telegramSafeUrl}">Open your WorldView Brief magazine</a>`
    : '';
  const briefFooterSlack = magazineUrl
    ? `\n\n${DIVIDER}\n\n📖 <${slackSafeUrl}|Open your WorldView Brief magazine>`
    : '';
  const briefFooterDiscord = magazineUrl
    ? `\n\n${DIVIDER}\n\n📖 [Open your WorldView Brief magazine](${magazineUrl})`
    : '';
  if (!aiSummary) {
    return {
...
<a href="${escapedUrl}" style="display:inline-block;background:#f2ede4;color:#0a0a0a;text-decoration:none;font-weight:700;font-size:14px;letter-spacing:0.08em;padding:14px 22px;border-radius:4px;">Open your WorldView Brief →</a>
...
async function composeAndStoreBriefForUser(userId, annotated, insightsNumbers, digestFor, nowMs) {
  ...
  const magazineUrl = await signBriefUrl({
    userId,
    issueDate: issueSlot,
    baseUrl: WORLDVIEW_PUBLIC_BASE_URL,
