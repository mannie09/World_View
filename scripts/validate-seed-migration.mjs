 ...
 *
 * Requires: Referer header from trusted origin OR X-WorldView-Key header.
 * Uses api.worldview.app by default.
 */

const BASE_URL = process.argv.includes('--base-url')
  ? process.argv[process.argv.indexOf('--base-url') + 1]
  : 'https://api.worldview.app';

const ORIGIN = 'https://worldview.app';
...
const API_KEY = process.env.WORLDVIEW_KEY || '';
...
async function fetchEndpoint(endpoint) {
  ...
  const resp = await fetch(url, {
    headers: {
      ...
      ...(API_KEY ? { 'X-WorldView-Key': API_KEY } : {}),
    },
