...
async function bootEmbed(): Promise<void> {
  ...
  try {
    ...
    attribution.href = buildWorldViewAttributionUrl(new URL('/dashboard', window.location.origin).toString(), getReferrerHost());
    attribution.target = '_blank';
    attribution.rel = 'noopener noreferrer';
    attribution.textContent = 'Live map by WorldView';
    ...
    mountError(root, 'WorldView map embed could not load.');
