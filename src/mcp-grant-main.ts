 ...
 * a "Sign in with WorldView Pro" CTA on `api/oauth/authorize.js`).
 *
 * Flow on this page:
 ...
 *      `https://api.worldview.app/oauth/authorize-pro?...` — the
...
try {
  const savedTheme = localStorage.getItem('worldview-theme');
...
function errorCodeToMessage(code: string | undefined): string {
  switch (code) {
    ...
    case 'INSUFFICIENT_TIER':
      return 'A WorldView Pro subscription is required to authorize MCP clients.';
...
async function onAuthorizeClick(nonce: string): Promise<void> {
  ...
  // Defense-in-depth: the apex page MUST navigate only to api.worldview.app.
  ...
  if (target.origin !== 'https://api.worldview.app') {
