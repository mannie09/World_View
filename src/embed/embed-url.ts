...
export function buildEmbedMapUrl(
  ...
  const url = new URL(baseUrl, 'https://www.worldview.app');
...
export function buildEmbedIframeSnippet(url: string, options: { width?: string; height?: string } = {}): string {
  ...
  return `<iframe src="${escapeAttribute(url)}" title="WorldView live map" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" style="width:${width};height:${height}px;border:0;display:block" allowfullscreen></iframe>`;
}

export function buildWorldViewAttributionUrl(baseUrl: string, referrerHost: string | null): string {
  const url = new URL(baseUrl, 'https://www.worldview.app');
