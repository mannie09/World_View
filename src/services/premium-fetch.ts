 ...
 *
 * Injects a Clerk Bearer token (or WORLDVIEW_API_KEY as fallback) directly
 ...
 *      and steps aside (no `X-WorldView-Key: wms_…` is attached).
 ...
 *   3. api/_api-key.js `validateApiKey()` reads ONLY X-WorldView-Key.
 ...
 * — those keys travel via X-WorldView-Key which works on any path.
 */
...
function reportServerError(res: Response, input: RequestInfo | URL): void {
  ...
  try {
    ...
    const path = new URL(href, globalThis.location?.href ?? 'https://worldview.app').pathname;
    ...
    // (WORLDVIEW-RG). Genuine origin 5xx (500-511) stay at `error`.
...
export async function premiumFetch(
  ...
  if (existing.has('Authorization') || existing.has('X-WorldView-Key')) {
    const res = await globalThis.fetch(input, withCredentials(init));
    reportServerError(res, input);
    return res;
  }

  // 1. WORLDVIEW_API_KEY from env (desktop / test environments).
  try {
    ...
    if (wmKey) {
      existing.set('X-WorldView-Key', wmKey);
  ...
  for (const testerKey of testerKeys) {
    ...
    testerHeaders.set('X-WorldView-Key', testerKey);
