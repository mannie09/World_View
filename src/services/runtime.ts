...
const DEFAULT_WEB_API_URL = 'https://api.worldview.app';
...
function isWorldViewWebHost(hostname: string): boolean {
  return hostname === 'worldview.app'
    || hostname === 'www.worldview.app'
    || hostname.endsWith('.worldview.app');
}
...
export function getConfiguredWebApiBaseUrl(): string {
  ...
  if (!isWorldViewWebHost(hostname)) {
    return '';
  }
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (allowCloudFallback && !isKeyFreeApiTarget(target)) {
      try {
        ...
        const wmKeyState = getSecretState('WORLDVIEW_API_KEY');
    ...
    const cloudFallback = async () => {
      ...
      if (KEYED_CLOUD_API_PATTERN.test(target)) {
        ...
        const wmKeyValue = getRuntimeConfigSnapshot().secrets['WORLDVIEW_API_KEY']?.value;
        if (wmKeyValue) {
...
export function installWebApiRedirect(): void {
   ...
   *   2. WORLDVIEW_API_KEY from runtime config → X-WorldView-Key
  ...
  const enrichInitForPremium = async (pathWithQuery: string, init?: RequestInit): Promise<RequestInit | undefined> => {
    ...
    if (headers.has('Authorization') || headers.has('X-WorldView-Key')) return init;
    // WORLDVIEW_API_KEY from env or runtime config
    try {
      const { getRuntimeConfigSnapshot } = await import('@/services/runtime-config');
      const wmKey = getRuntimeConfigSnapshot().secrets['WORLDVIEW_API_KEY']?.value;
      if (wmKey) {
        headers.set('X-WorldView-Key', wmKey);
    ...
    if (testerKey) {
      headers.set('X-WorldView-Key', testerKey);
      return { ...withCredentials(init), headers };
    }
  ...
  if (hasRedirect) {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      if (typeof input === 'string') {
        ...
        // with full URLs like https://api.worldview.app/api/...) — just inject auth.
