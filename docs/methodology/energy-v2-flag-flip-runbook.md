...
- `https://www.worldview.app/api/resilience/v1/get-runtime-manifest`
  returned HTTP 200 with `formulaTag: "pc"` and
  `constructVersions.energy: "v2"` when requested with a browser-like
  user agent.
- `https://www.worldview.app/api/health` returned HTTP 200. The overall
...
authentication required` without `WORLDVIEW_API_KEY`. The dedicated
...
```bash
export API_BASE=https://www.worldview.app
export WORLDVIEW_API_KEY=<pro-api-key>
...
requires WORLDVIEW_API_KEY because freeze-resilience-ranking verifies score
...
3. **Bump the score-cache prefix.** Add a new commit to this branch
   ...
   `server/worldview/resilience/v1/_shared.ts`. This guarantees the
...
4. **Flip the flag in production.**
   ...
   ```bash
   curl -s https://worldview.app/api/resilience/v1/get-runtime-manifest \
