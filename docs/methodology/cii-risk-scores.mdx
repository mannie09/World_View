...
(proto `worldview.intelligence.v1.GetRiskScoresResponse`).

These weights are **editorial** — authored by the WorldView intelligence
...
> version ([#4200](https://github.com/mannie09/World_View/issues/4200)), and
...
> [#3789](https://github.com/mannie09/World_View/issues/3789) by keeping the
...
> `server/worldview/intelligence/v1/_risk-config.ts`, an update to this
...
`server/worldview/intelligence/v1/get-risk-scores.ts`. Each `CiiScore`
therefore exposes:
...
are all defined in
[`server/worldview/intelligence/v1/_risk-config.ts`](../../server/worldview/intelligence/v1/_risk-config.ts).
Their **rationale**:
