...
- **Every merge touching a scorer file** in `server/worldview/resilience/v1/_dimension-scorers.ts`,
  `server/worldview/resilience/v1/_shared.ts`, or a scorer-feeding
...
```bash
# Online (hits the live API; requires WORLDVIEW_API_KEY)
WORLDVIEW_API_KEY=wm_xxx \
API_BASE=https://api.worldview.app \
...
| Var | Default | Notes |
|---|---|---|
| `API_BASE` | (required unless FIXTURE set) | e.g. `https://api.worldview.app` |
| `WORLDVIEW_API_KEY` | (required unless FIXTURE set) | resilience RPCs are in `PREMIUM_RPC_PATHS` |
