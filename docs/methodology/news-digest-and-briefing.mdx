...
description: "How WorldView collects RSS/Atom feeds, classifies news, scores importance, tracks stories, filters briefs, uses LLMs, and controls bias."
---

## Scope

This page is the canonical public methodology for the news-feed digest,
daily digest notifications, and WorldView Brief. It covers the automated
...
Feed inventory lives in `server/worldview/news/v1/_feeds.ts`.
...
`energy` is a site and client-feed variant (`energy.worldview.app`), but it
...
`server/worldview/news/v1/_classifier.ts`.
...
WorldView Brief uses two LLM surfaces:
...
- Feed inventory and digest build: `server/worldview/news/v1/_feeds.ts`,
  `server/worldview/news/v1/list-feed-digest.ts`
- Keyword classifier: `server/worldview/news/v1/_classifier.ts`
- API contract: `proto/worldview/news/v1/list_feed_digest.proto`
