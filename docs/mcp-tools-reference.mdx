...
description: "Per-tool reference for every MCP tool exposed by WorldView — parameters, freshness, API endpoints, and example calls."
---
...
If you've completed the OAuth flow instead, replace `-H "X-WorldView-Key: $WM_KEY"` with `-H "Authorization: Bearer $TOKEN"` in each example.
...
Real-time equity quotes, commodity prices (including gold futures GC=F), crypto prices, forex FX rates (USD/EUR, USD/JPY etc.), sector performance, ETF flows, and Gulf market quotes from WorldView's curated bootstrap cache.
...
- **Kind:** live RPC — proxies a fetch to the WorldView API on each call. Edge-runtime timeout: **8.0s**.
...
- **Kind:** live RPC — proxies a fetch to the WorldView API on each call. Worst-case total budget **~24s** (2s context-digest fetch + 22s brief generation, sequential).
...
AI-classified geopolitical threat news summaries, GDELT intelligence signals, cross-source signals, and security advisories from WorldView's intelligence layer.
...
- **Kind:** live RPC — proxies a fetch to the WorldView API on each call. Edge-runtime timeout: **25.0s**.
...
- **Kind:** live RPC — proxies a fetch to the WorldView API on each call. Worst-case total budget **~24s** (6s digest fetch + 18s LLM summarization, sequential).
...
AI-generated geopolitical and economic forecasts from WorldView's predictive models. Covers upcoming risk events and probability assessments.
