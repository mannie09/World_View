...
description: "Desktop cloud fallback is gated on a WORLDVIEW_API_KEY. Without a valid key, the desktop app operates local-only (sidecar). A registration form collects emails via Convex DB for future key distribution."
---
## Overview

Desktop cloud fallback is gated on a `WORLDVIEW_API_KEY`. Without a valid key, the desktop app operates local-only (sidecar). A registration form collects emails via Convex DB for future key distribution.
...
|----------|-------------|---------|
| `WORLDVIEW_VALID_KEYS` | Comma-separated list of valid API keys | `wm_abc123def456,wm_xyz789` |
...
Add to `WORLDVIEW_VALID_KEYS` in Vercel dashboard (comma-separated, no spaces).
...
- `installRuntimeFetchPatch()` checks `WORLDVIEW_API_KEY` before allowing cloud fallback
...
- `api/_api-key.js` validates `X-WorldView-Key` header on sebuf routes
- **Origin-aware**: desktop origins (`tauri.localhost`, `tauri://`, `asset://`) require a key
- Web origins (`worldview.app`) pass through without a key
- Non-desktop origin with key header: key is still validated
- Invalid key returns `401 { error: "Invalid API key" }`

### CORS

`X-WorldView-Key` is allowed in both `server/cors.ts` and `api/_cors.js`.
...
- [ ] Set `WORLDVIEW_VALID_KEYS` in Vercel
...
| `src/services/runtime-config.ts` | `WORLDVIEW_API_KEY` type, validation, `secretsReady` |
...
| `server/cors.ts` / `api/_cors.js` | CORS headers with `X-WorldView-Key` |
| `src/components/WorldViewTab.ts` | Settings UI for key + registration |
