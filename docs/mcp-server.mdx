...
<Info>
**Pro and API tiers can both connect via OAuth — no API key required.** Pro subscribers click _"Sign in with WorldView Pro"_ on the consent page; API Starter / Business / Enterprise users may sign in the same way OR paste a `wm_…` key. Free-tier users see a 401 at the OAuth step.
</Info>
...
| Endpoint | Purpose |
|----------|---------|
| `https://worldview.app/mcp` | JSON-RPC server (Streamable HTTP transport; JSON responses by default, SSE responses when clients advertise `text/event-stream`; `initialize` defaults to protocol `2025-03-26`) |
| `https://api.worldview.app/api/oauth/register` | Dynamic Client Registration (RFC 7591) |
| `https://api.worldview.app/api/oauth/authorize` | OAuth 2.1 authorization endpoint (PKCE required) |
| `https://api.worldview.app/api/oauth/token` | Token endpoint (authorization_code + refresh_token) |
| `https://api.worldview.app/.well-known/oauth-authorization-server` | AS metadata (RFC 8414) |
| `https://worldview.app/.well-known/oauth-protected-resource` | Resource server metadata (RFC 9728) |

Server identifier: `worldview` v1.10.0.

### Protocol negotiation

WorldView's static server card at `/.well-known/mcp/server-card.json` advertises protocol version `2025-06-18`, matching the current published capability card. The live `initialize` handshake is intentionally more conservative during the rollout:
...
WorldView supports the Streamable HTTP POST flow with either JSON or SSE responses:
...
Pro subscribers (and API Starter+ users who'd rather not paste a key) can authorize MCP clients via their existing WorldView account — no API key needed.

1. **Add the server URL** in your MCP client. The canonical entrypoint is:

   ```
   https://api.worldview.app/mcp
   ```

   (`https://worldview.app/mcp` works too — it proxies the same handler.)

2. **Click "Sign in with WorldView Pro"** on the consent page. This is the default CTA. You'll bounce through `worldview.app/mcp-grant` (Clerk-protected — sign in if needed), then back to `api.worldview.app/oauth/authorize-pro`, then to your client's redirect.
...
- Manage connected clients in **[Settings → Connected MCP clients](https://worldview.app/settings)**.
...
|------|------------|------------|-----------------------|-------|
| **Free** | ❌ No | — | — | OAuth flow returns 401 `INSUFFICIENT_TIER`. Upgrade at [worldview.app/pro](https://www.worldview.app/pro). |
...
In addition to the 39 tools, WorldView exposes MCP prompts and resources so clients can discover common workflows and address stable data slices without inventing tool plans from scratch.
...
|--------------------|--------------|
| `worldview://countries/{iso2}/risk` | Country risk score, component breakdown, travel advisory, and sanctions exposure. `{iso2}` is lowercase alpha-2, for example `de` or `us`. |
| `worldview://chokepoints/{slug}/status` | Chokepoint transit summary and risk narrative. `{slug}` is one of the published kebab-case chokepoint slugs, such as `suez`, `strait-of-hormuz`, or `bab-el-mandeb`. |
| `worldview://seed-meta/freshness` | Market-data bootstrap freshness envelope only: `cached_at` and `stale`. |
| `worldview://markets/{symbol}/quote` | Single-symbol market quote slice. `{symbol}` is uppercase, for example `AAPL`, `GC=F`, or `BTC-USD`. |
...
Server-side with a direct API key — send it as `X-WorldView-Key`, **not** as a bearer token.
...
Seed-level health per key: [status.worldview.app](https://status.worldview.app/).
...
- [Pro features](https://www.worldview.app/pro)
