...
- **Pro** — sign in with your WorldView account, no key to manage. 50 quota-consuming calls per UTC day.
...
Upgrade or generate a key at [worldview.app/pro](https://www.worldview.app/pro). The rest of this guide assumes Claude Desktop + OAuth (the simplest path); if you'd rather paste a `wm_…` key into a `curl` script, jump to [Server-side curl](#server-side-curl) at the bottom.

## 2. Add the server to Claude Desktop

Edit `~/Library/Application Support/Claude/claude_desktop_config.json` (macOS) or the Windows equivalent at `%APPDATA%\Claude\claude_desktop_config.json` and add the `worldview` entry:
...
Restart Claude Desktop. The first time you mention WorldView in a chat, Claude pops the OAuth consent screen — click **Sign in with WorldView Pro**, authenticate in your browser, and the token is stored locally by Claude. No API key ever touches your filesystem.
...
```bash
export WM_KEY="wm_0123456789abcdef0123456789abcdef01234567"  # API Starter+ key from worldview.app/settings
...
The `wm_…` user API key goes in `X-WorldView-Key`, **not** as a `Bearer` token — sending it as a bearer fails OAuth resolution and returns `401 invalid_token`. If you already have an OAuth access token from `/api/oauth/token`, use `Authorization: Bearer $TOKEN` instead and drop the `X-WorldView-Key` header.
...
- **[Authentication overview](/authentication)** — when to use an API key in `X-WorldView-Key` vs. OAuth vs. browser session.
