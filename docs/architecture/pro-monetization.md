...
Products are Dodo `productId`s stored client-side in `pro-test/src/generated/tiers.json` and served at runtime from `https://api.worldview.app/api/product-catalog`:
...
- **Enterprise** — `mailto:enterprise@worldview.app` (contact sales).
...
1. Validates `returnUrl` against an allow-listed set of worldview.app origins.
...
- **Full-page return** (fallback / `/pro` path): Dodo redirects to `worldview.app/?subscription_id=...&status=active`. `src/services/checkout-return.ts:handleCheckoutReturn()` reads params, cleans the URL, returns success boolean.
...
- **Share link**: `https://worldview.app/pro?ref=<code>`.
