...
- There is no `getSimulationPackage(runId)` RPC handler in `server/worldview/forecast/v1/`
...
Create `server/worldview/forecast/v1/get-simulation-package.ts` that reads from R2 using `buildSimulationPackageKey(runId, generatedAt)`. Follows the same pattern as the deep-snapshot replay handler.
...
- [ ] `getSimulationPackage(runId)` RPC handler exists in `server/worldview/forecast/v1/`
...
- New file needed: `server/worldview/forecast/v1/get-simulation-package.ts`
- Wire in: `server/worldview/handler.ts` (gateway registration)
- Follow pattern of: `server/worldview/forecast/v1/get-forecasts.ts`
