...
- See MEMORY: `feedback_is_caller_premium_trusted_origin.md` — the API key is extractable from `X-WorldView-Key`
...
```ts
const apiKey = req.headers.get('x-worldview-key');
...
## Acceptance Criteria
- [ ] Rate limit key uses `X-WorldView-Key` when present
