...
- **`server/worldview/intelligence/v1/get-country-intel-brief.ts:38`** — `frameworkRaw = req.framework.slice(0, 2000)` — truncation only, no sanitization
- **`server/worldview/intelligence/v1/deduct-situation.ts`** — same
- **`server/worldview/news/v1/summarize-article.ts`** — same
...
## Technical Details
- Files: `server/_shared/llm.ts`, `server/worldview/intelligence/v1/get-country-intel-brief.ts`, `server/worldview/intelligence/v1/deduct-situation.ts`, `server/worldview/news/v1/summarize-article.ts`
