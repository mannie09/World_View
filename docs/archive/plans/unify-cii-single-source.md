...
mislabel is a symptom, not the bug. The bug: **WorldView runs two CII engines that
...
|---|---|---|
| File | `src/services/country-instability.ts` | `server/worldview/intelligence/v1/get-risk-scores.ts` |
...
`src/generated/server/worldview/intelligence/v1/service_server.ts` +
`src/generated/client/worldview/intelligence/v1/service_client.ts`. Either way the
