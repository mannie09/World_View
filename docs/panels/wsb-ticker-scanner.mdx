...
**PRO (hard-locked).** `premium: 'locked'` in variant registrations. `isPanelEntitled` unlocks it when **either** a valid `WORLDVIEW_API_KEY` is present **or** `isPro` is true (`wsb-ticker-scanner` is in the `apiKeyPanels` allowlist at `src/config/panels.ts:973`).
