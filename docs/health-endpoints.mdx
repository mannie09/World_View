...
description: "WorldView exposes two health endpoints for monitoring data pipeline integrity. Both run on the Vercel Edge Runtime and query Redis (Upstash) to assess system state."
---
...
```bash
# Full health check
curl -s https://api.worldview.app/api/health | jq .

# Compact (problems only)
curl -s "https://api.worldview.app/api/health?compact=1" | jq .

# UptimeRobot / monitoring: check HTTP status code
curl -o /dev/null -s -w "%{http_code}" https://api.worldview.app/api/health
...
```bash
curl -s https://api.worldview.app/api/seed-health \
  -H "Origin: https://worldview.app" | jq .
```
...
Point the keyword monitor at `https://api.worldview.app/api/health?compact=1` and alert when the compact token `"status":"HEALTHY"` (no space after the colon) is **absent** from the response body. Compact mode serializes with no indentation, so this exact token is stable regardless of formatting.
...
STATUS=$(curl -s "https://api.worldview.app/api/health?compact=1")
