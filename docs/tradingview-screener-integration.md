...
**Purpose:** Reference document for extending WorldView's finance hub with TradingView Screener data.
...
The TypeScript library is the one relevant for WorldView. It has full parity with the Python version (41/41 operations pass) plus a self-hosted REST server option.
...
WorldView's data flow: `Railway seeds Redis → Vercel reads Redis → Frontend RPC`
...
```typescript
// server/worldview/market/v1/list-tv-stock-screener.ts
...
### WorldView-Specific Recommendations
...
| **No auth initially** | Delayed data is fine for WorldView's use case |
...
- [ ] Create `proto/worldview/market/v1/list_tv_stock_screener.proto`
- [ ] Create `server/worldview/market/v1/list-tv-stock-screener.ts`
