...
- Every edge response includes `x-vercel-id` and `x-worldview-deploy` headers — include those when reporting issues.
- Sentry alerts forward to [status.worldview.app](https://status.worldview.app/).
