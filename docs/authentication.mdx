...
description: "How user authentication, premium panel gating, and server-side session enforcement work in WorldView."
---

WorldView uses [Clerk](https://clerk.com) for authentication. The auth system gates premium panels behind sign-in and tier checks, and enforces session-based access on server-side API endpoints via local JWT verification.
...
Desktop users with a valid `WORLDVIEW_API_KEY` in the Tauri keychain bypass all panel gating. The existing API key flow is unaffected — bearer tokens are a **second auth path**, not a replacement.
...
1. **Static API key** — `X-WorldView-Key` header (existing flow, unchanged)
