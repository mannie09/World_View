...
| Pattern | Matches |
| --- | --- |
| `(*.)?worldview.app` | Production + subdomains (`tech.`, `finance.`, etc.) |
| `worldview-*-elie-*.vercel.app` | Vercel preview deploys |
...
- `Content-Type`
- `Authorization`
- `X-WorldView-Key` (API key for desktop/third-party access). See [API Key Gating](/api-key-deployment) for key management details.
- `X-Api-Key`
- `X-Widget-Key`
- `X-Pro-Key`
