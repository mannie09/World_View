...
- **Cloud fallback** — when a local API handler fails or is missing, requests transparently fall through to the cloud deployment (worldview.app) with origin headers stripped
...
WorldView desktop uses a runtime configuration schema with per-feature toggles and secret-backed credentials.
...
- `OLLAMA_API_URL`
- `OLLAMA_MODEL`
- `WORLDVIEW_API_KEY` — gates cloud fallback access (min 16 chars)
- `WTO_API_KEY`
- `AVIATIONSTACK_API`
- `ICAO_API_KEY`
- `UCDP_ACCESS_TOKEN`
...
- WorldView API key: cloud fallback is blocked; desktop operates local-only.
...
The desktop app checks for new versions by polling `worldview.app/api/version` — once at startup (after a 5-second delay) and then every 6 hours. When a newer version is detected (semver comparison), a non-intrusive update badge appears with a direct link to the GitHub Release page.
