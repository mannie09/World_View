...
| **IndexedDB** | `worldview_db` | Client-side storage for playback snapshots and temporal baseline data. |
...
WorldView ships as three product variants from a single codebase. Each variant surfaces a different subset of panels, map layers, and data sources.

| Variant | Domain | Focus |
|---|---|---|
| `full` | worldview.app | Geopolitics, military, OSINT, conflicts, markets |
| `tech` | tech.worldview.app | AI/ML, startups, cybersecurity, developer tools |
| `finance` | finance.worldview.app | Markets, trading, central banks, macro indicators |
...
```
localStorage('worldview-variant')  →  import.meta.env.VITE_VARIANT  →  default 'full'
```
...
graph TD
    ...
    subgraph LayerConfig["Layer Configuration"]
        ...
        UserPref["localStorage overrides<br/>(worldview-layers)"]
...
2. **User localStorage** — Stored under the key `worldview-layers`. Users toggle layers in the map controls UI, and their preferences persist across sessions.
...
WorldView employs a five-tier caching strategy to minimise API costs, reduce latency, and enable offline operation.
...
graph TD
    ...
    subgraph Tier5["Tier 5: Persistent Cache"]
        ...
        LSFallback["localStorage fallback<br/>prefix: worldview-persistent-cache:"]
        PC --> TauriInvoke
        PC --> LSFallback
    end
...
The `worldview_db` IndexedDB database contains two object stores:
...
**2. Feature Toggles** — 14 `RuntimeFeatureId` values stored in localStorage under the key `worldview-runtime-feature-toggles`:
...
WorldView runs machine-learning inference directly in the browser using ONNX Runtime Web via Transformers.js, with API-based fallbacks for constrained devices.
...
WorldView uses a circuit-breaker pattern to manage transient failures across its many data sources, preventing cascade failures and providing graceful degradation.
