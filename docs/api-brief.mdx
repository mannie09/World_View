...
WorldView composes a per-user intelligence brief on Railway, stores each
...
Single-shot completion endpoint used by embedded widget iframes. Auth via `X-WorldView-Key` (partner keys). Rate-limited per key.
