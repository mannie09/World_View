---
title: "Quickstart"
description: "Make your first WorldView API call in under a minute — curl, JavaScript, and Python."
---

## 1. Get an API key

PRO subscribers get an API key from [worldview.app/pro](https://www.worldview.app/pro). User-issued keys are exactly `wm_` followed by 40 lowercase hex characters.
...
```bash
curl -s 'https://api.worldview.app/api/conflict/v1/list-conflict-events?limit=5' \
  -H 'X-WorldView-Key: wm_0123456789abcdef0123456789abcdef01234567'
```
...
const res = await fetch(
  'https://api.worldview.app/api/market/v1/get-quotes?symbols=AAPL,TSLA',
  { headers: { 'X-WorldView-Key': process.env.WM_KEY } }
);
...
r = httpx.get(
    "https://api.worldview.app/api/economic/v1/get-fred-series",
    params={"series_id": "FEDFUNDS"},
    headers={"X-WorldView-Key": os.environ["WM_KEY"]},
)
r.raise_for_status()
print(r.json())
```
...
```bash
curl -s 'https://api.worldview.app/api/bootstrap' \
  -H 'X-WorldView-Key: wm_0123456789abcdef0123456789abcdef01234567'
```
...
```
https://api.worldview.app/api/mcp
```
