...
description: "Bootstrap, health, versioning, cache-purge, and user preferences — the plumbing endpoints every WorldView client talks to."
---
...
- **Auth**: browser `wm-session` cookie OR `X-WorldView-Key`
...
Returns the latest **GitHub Release** of `mannie09/World_View`. Used by the desktop app to detect a newer published release and prompt the user to update. It is **not** the currently-deployed Vercel commit.

```json
{
  "version": "2.6.7",
  "tag": "v2.6.7",
  "url": "https://github.com/mannie09/World_View/releases/tag/v2.6.7",
  "prerelease": false
}
```
...
Redirects to the matching asset on the latest GitHub release of `mannie09/World_View`. Returns `302` to the asset URL on success, or `302` to [releases/latest](https://github.com/mannie09/World_View/releases/latest) on any failure (unknown platform, no match, GitHub error).
...
- `X-WorldView-Desktop-Timestamp`: Unix epoch milliseconds, within 5 minutes of server time.
- `X-WorldView-Desktop-Signature`: `sha256=<hex HMAC-SHA256>`.
