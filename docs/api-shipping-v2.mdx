...
The v2 shipping API is a **PRO-gated** read + webhook-subscription surface on top of WorldView's chokepoint registry and AIS tracking data.

<Info>
All v2 shipping endpoints require `X-WorldView-Key` (server-to-server). Browser origins are **not** trusted here — `validateApiKey` runs with `forceKey: true`.
</Info>
...
Auth: `X-WorldView-Key` (forceKey: true) + PRO. Returns `401` / `403` otherwise.
