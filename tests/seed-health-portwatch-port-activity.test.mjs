...
const originalEnv = {
  ...
  WORLDVIEW_VALID_KEYS: process.env.WORLDVIEW_VALID_KEYS,
...
process.env.WORLDVIEW_VALID_KEYS = 'test-key';
...
async function readSeedHealth() {
  const req = new Request('https://api.worldview.app/api/seed-health', {
    headers: { 'X-WorldView-Key': 'test-key' },
  });
