...
describe('mission preset shell integration', () => {
  it('applies a preset through the real manager path and resets state, storage, layers, map view, and URL to defaults', async () => {
    ...
    assert.equal(readJsonStorage<MapLayers>('worldview-layers').tradeRoutes, true);
    assert.equal(readJsonStorage<MapLayers>('worldview-layers').resilienceScore, true);
    ...
    assert.deepEqual(activeLayers(readJsonStorage<MapLayers>('worldview-layers')), baselineLayers);
  ...
  it('filters renderer-incompatible preset layers before persisting or running side effects on the mobile fallback renderer', async () => {
    ...
    assert.equal(readJsonStorage<MapLayers>('worldview-layers').resilienceScore, false);
  ...
  it('filters AIS before persisting a mission preset when AIS is not configured', async () => {
    ...
    assert.equal(readJsonStorage<MapLayers>('worldview-layers').ais, false);
