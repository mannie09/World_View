...
import { buildAnalystSystemPrompt } from '../server/worldview/intelligence/v1/chat-analyst-prompt.ts';
import { buildActionEvents, VISUAL_INTENT_RE } from '../server/worldview/intelligence/v1/chat-analyst-actions.ts';
import { postProcessAnalystHtml } from '../src/utils/analyst-markdown.ts';
import { buildWorldBrief, extractKeywords } from '../server/worldview/intelligence/v1/chat-analyst-context.ts';
import type { AnalystContext } from '../server/worldview/intelligence/v1/chat-analyst-context.ts';
...
// ---------------------------------------------------------------------------
// handler — edge wiring + pre-auth gates (WORLDVIEW-SV)
//
...
describe('api/chat-analyst handler — edge wiring + pre-auth gates', () => {
  ...
  it('returns 204 with CORS on OPTIONS preflight (no secrets / no Redis)', async () => {
    ...
    const req = new Request('https://api.worldview.app/api/chat-analyst', {
      method: 'OPTIONS',
  ...
  it('has a top-level error boundary that fails to a CORS-correct 503 (not an opaque platform 500)', () => {
    ...
    // side Sentry capture — the gap that left WORLDVIEW-SV diagnosable only
