...
const SERVER_FEEDS_PATH = resolve(__dirname, '../server/worldview/news/v1/_feeds.ts');
  ...
  for (const { clientConst, serverKey, knownGapsClientOnly } of VARIANTS) {
    test(`${serverKey}: every client key has a matching server key`, () => {
      ...
      assert.deepStrictEqual(
        ...
        `Fix: rename the server-side key in server/worldview/news/v1/_feeds.ts under\n` +
