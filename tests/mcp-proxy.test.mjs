...
process.env.WORLDVIEW_VALID_KEYS = ENTERPRISE_KEY;
...
function buildHeaders(origin, { authed = true, extra = {} } = {}) {
  ...
  if (authed) h['X-WorldView-Key'] = ENTERPRISE_KEY;
  return h;
}
...
function makeGetRequest(params = {}, origin = 'https://worldview.app', opts = {}) {
  const url = new URL('https://worldview.app/api/mcp-proxy');
...
function makePostRequest(body = {}, origin = 'https://worldview.app', opts = {}) {
  return new Request('https://worldview.app/api/mcp-proxy', {
    method: 'POST',
...
function makeOptionsRequest(origin = 'https://worldview.app') {
  return new Request('https://worldview.app/api/mcp-proxy', {
    method: 'OPTIONS',
    headers: { origin },
  });
...
describe('api/mcp-proxy', () => {
  ...
  describe('Auth gate', () => {
    it('returns 401 when no X-WorldView-Key is provided', async () => {
      const res = await handler(makeGetRequest({ serverUrl: 'https://mcp.example.com/mcp' }, 'https://worldview.app', { authed: false }));
    ...
    it('returns 401 for POST without key', async () => {
      const res = await handler(makePostRequest({ serverUrl: 'https://mcp.example.com/mcp', toolName: 'search' }, 'https://worldview.app', { authed: false }));
      assert.equal(res.status, 401);
    });
    ...
    it('rejects wm_ user keys when Convex validation cannot run / returns null', async () => {
      ...
      const req = new Request(url.toString(), {
        method: 'GET',
        headers: { origin: 'https://worldview.app', 'X-WorldView-Key': 'wm_user_abc123' },
      });
      const res = await handler(req);
      assert.equal(res.status, 401);
    });
  ...
  describe('CORS and method handling', () => {
    ...
    it('returns 405 for DELETE', async () => {
      const res = await handler(new Request('https://worldview.app/api/mcp-proxy', {
        method: 'DELETE',
        headers: { origin: 'https://worldview.app', 'X-WorldView-Key': ENTERPRISE_KEY },
      }));
  ...
  describe('GET /api/mcp-proxy (list tools)', () => {
    ...
    it('ignores invalid JSON in headers param', async () => {
      ...
      const req = new Request(url.toString(), { method: 'GET', headers: { origin: 'https://worldview.app', 'X-WorldView-Key': ENTERPRISE_KEY } });
      const res = await handler(req);
      assert.equal(res.status, 200);
    });
