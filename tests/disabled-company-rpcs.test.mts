...
import { listCompanySignals } from '../server/worldview/intelligence/v1/list-company-signals';
import { getCompanyEnrichment } from '../server/worldview/intelligence/v1/get-company-enrichment';
import { ValidationError } from '../src/generated/server/worldview/intelligence/v1/service_server';

const __dirname = dirname(fileURLToPath(import.meta.url));
const root = resolve(__dirname, '..');

const signalsSrc = readFileSync(
  resolve(root, 'server/worldview/intelligence/v1/list-company-signals.ts'),
  'utf-8',
);
const enrichmentSrc = readFileSync(
  resolve(root, 'server/worldview/intelligence/v1/get-company-enrichment.ts'),
  'utf-8',
);
