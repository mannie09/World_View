 ...
 * `WORLDVIEW_API_KEY` (a server-side platform credential) into
 ...
 *      bundle. `WORLDVIEW_API_KEY` has no prefix → invisible to
 *      `readEnvSecret()` at runtime in web builds.
 *
 *   2. No entry in `RUNTIME_FEATURES.requiredSecrets` references
 *      `WORLDVIEW_API_KEY`, so `seedSecretsFromEnvironment()` never
 *      iterates over it — the key isn't even attempted.
 *
 *   3. `vite.config.ts` does not pass `WORLDVIEW_API_KEY` through
...
// of these grants access to worldview.app infrastructure. They are
...
describe('browser bundle secret guard (#3704)', () => {
  ...
  // always passes — even if `WORLDVIEW_API_KEY` were added to a
  ...
  //
  //   npm run build
  //   grep -r "WORLDVIEW_API_KEY" dist/  # must return zero hits
  //
