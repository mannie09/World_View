...
describe('embed URL contract', () => {
  ...
  it('builds a narrow embed URL with no main-app state params', () => {
    ...
    layers.ais = true;
    layers.weather = true;
    const url = buildEmbedMapUrl('https://www.worldview.app/embed?country=US&expanded=true', {
      layers,
  ...
  it('escapes iframe snippet attributes', () => {
    const snippet = buildEmbedIframeSnippet('https://www.worldview.app/embed?layers=weather&x="><script>');
    assert.ok(snippet.includes('title="WorldView live map"'));
