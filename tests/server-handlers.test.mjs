...
describe('getHumanitarianSummary handler', () => {
  const src = readSrc('server/worldview/conflict/v1/get-humanitarian-summary.ts');
...
describe('humanitarian_summary.proto', () => {
  const proto = readSrc('proto/worldview/conflict/v1/humanitarian_summary.proto');
...
describe('LLM prompt political context (LOW-1)', () => {
  const src = readSrc('server/worldview/news/v1/_shared.ts');
...
describe('getCacheKey determinism', () => {
  ...
  const sharedSrc = readSrc('server/worldview/news/v1/_shared.ts');
...
describe('getVesselSnapshot caching (HIGH-1)', () => {
  const src = readSrc('server/worldview/maritime/v1/get-vessel-snapshot.ts');
...
describe('getSimulationOutcome handler', () => {
  const src = readSrc('server/worldview/forecast/v1/get-simulation-outcome.ts');
