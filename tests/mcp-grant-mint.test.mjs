 ...
 *                                    api.worldview.app/oauth/authorize-pro
...
function makePostReq(body) {
  return new Request('https://worldview.app/api/internal/mcp-grant-mint', {
    method: 'POST',
...
function makeGetReq(nonce) {
  const url = nonce !== undefined
    ? `https://worldview.app/api/internal/mcp-grant-context?nonce=${encodeURIComponent(nonce)}`
    : `https://worldview.app/api/internal/mcp-grant-context`;
...
describe('mintGrantHandler', () => {
  ...
  it('happy path: returns redirect to https://api.worldview.app/oauth/authorize-pro with valid grant', async () => {
    ...
    assert.equal(u.origin, 'https://api.worldview.app');
  ...
  it('returns 405 on non-POST', async () => {
    ...
    const req = new Request('https://worldview.app/api/internal/mcp-grant-mint', { method: 'GET' });
  ...
  it('returns 400 INVALID_REQUEST on non-JSON body', async () => {
    const { deps } = makeMintDeps();
    const req = new Request('https://worldview.app/api/internal/mcp-grant-mint', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: 'Bearer fake-jwt' },
      body: 'not json {',
    });
...
describe('grantContextHandler', () => {
  ...
  it('returns 405 on non-GET', async () => {
    const { deps } = makeContextDeps();
    const req = new Request('https://worldview.app/api/internal/mcp-grant-context?nonce=x', {
      method: 'POST',
      headers: { Authorization: 'Bearer fake-jwt' },
    });
